class Easing {
	...
	static outQuad(t) { return t * (2 - t); }
	static inOutQuad(t) { return t < .5 ? 2 * t * t : -1 + (4 - 2 * t) * t; }
	static inCubic(t) { return t * t * t; }
	static outCubic(t) { return (--t) * t * t + 1; }
	static inOutCubic(t) { return t < .5 ? 4 * t * t * t : (t - 1) * (2 * t - 2) * (2 * t - 2) + 1; }
	static inQuart(t) { return t * t * t * t; }
	static outQuart(t) { return 1 - (--t) * t * t * t; }
	static inOutQuart(t) { return t < .5 ? 8 * t * t * t * t : 1 - 8 * (--t) * t * t * t; }
	static inQuint(t) { return t * t * t * t * t; }
	static outQuint(t) { return 1 + (--t) * t * t * t * t; }
	static inOutQuint(t) { return t < .5 ? 16 * t * t * t * t * t : 1 + 16 * (--t) * t * t * t * t; }
	static inSine(t) { return -1 * Math.cos(t / 1 * (Math.PI * 0.5)) + 1; }
	static outSine(t) { return Math.sin(t / 1 * (Math.PI * 0.5)); }
	static inOutSine(t) { return -1 / 2 * (Math.cos(Math.PI * t) - 1); }
	static inExpo(t) { return (t == 0) ? 0 : Math.pow(2, 10 * (t - 1)); }
	static outExpo(t) { return (t == 1) ? 1 : (-Math.pow(2, -10 * t) + 1); }
	static inOutExpo(t) {
		if (t == 0) return 0;
		if (t == 1) return 1;
	...
	static inCirc(t) { return -1 * (Math.sqrt(1 - t * t) - 1); }
	static outCirc(t) { return Math.sqrt(1 - (t = t - 1) * t); }
	static inOutCirc(t) {
	...
	static outBounce(t) { return 1 - this.inBounce(1 - t); }
	static inBounce(t) {
		if ((t /= 1) < (1 / 2.75)) {
			return (7.5625 * t * t);
		} else if (t < (2 / 2.75)) {
...
Easing.softInElastic = Easing.cubicBezier(.08, 1.07, .55, 1.26);
Easing.softOutElastic = Easing.cubicBezier(.45, -0.26, .8, -0.07);
...
class Polimot {
	...
	constructor(...timelines) {
		...
		this._.detectEasing = function (easing, saveMode = true) {
			if (easing) {
				switch (typeof easing) {
					...
					case 'object':
						if (easing === null) {
							...
							return Easing.cubicBezier(...easing);
						}
						break;
					default:
						return Easing.linear;
				...
				if (saveMode) return Easing.linear;
			}
		};
		...
		this._.makeTimeline = function (timeline, idxTimeline, reference) {
		...
		this._.compile = function () {
			...
			timelines.forEach(function (timeline, idxTimeline) {
	...
	play(startLoop = 0) {
		...
		let animate = function (time) {
			let currentTime = this._.direction === 'reverse' ? Math.max(Math.min(start - time, this._.duration), 0) : Math.min(Math.max(time - start, 0), this._.duration);
